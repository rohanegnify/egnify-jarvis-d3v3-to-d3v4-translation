...
var new_scaling = d3.scale.ordinal()
...
canvas.append("g").attr("class", "x axis")
    ...
    .attr('font-size', 15)
    .call(d3.svg.axis().scale(new_scaling).orient("bottom").tickSize(0)
        .tickPadding(6));
...
function update(freqData) {
    ...
    var scaling = d3.scale.ordinal()
        .domain(freqData.map(function (d) {
            return d.State;
        }))
        .rangeRoundBands([0, width], 0.1);

    var scalingy = d3.scale.linear()
